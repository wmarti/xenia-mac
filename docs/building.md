...
You will also need Clang and some development libraries. To get them on an Ubuntu system:

```bash
sudo apt update
sudo apt-get install clang libgtk-3-dev libpthread-stubs0-dev liblz4-dev libx11-dev libx11-xcb-dev libxcb1-dev libxcb-xkb-dev libvulkan-dev libsdl2-dev libiberty-dev libc++-dev libc++abi-dev
```
