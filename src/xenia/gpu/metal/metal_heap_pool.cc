...
void MetalHeapPool::Shutdown() {
  for (auto& entry : heaps_) {
    if (entry.heap) {
      TrackMetalHeapReleased(entry.size);
      entry.heap->release();
      entry.heap = nullptr;
    }
  }
...
MTL::Texture* MetalHeapPool::CreateTexture(
...
MTL::Heap* MetalHeapPool::GetHeapForSize(size_t size, size_t alignment) {
  for (auto& entry : heaps_) {
    ...
    size_t available = size_t(entry.heap->maxAvailableSize(
        static_cast<NS::UInteger>(alignment)));
    if (available >= size) {
      return entry.heap;
    }
  }
  ...
  if (max_heap_bytes_ &&
      total_heap_bytes_ > max_heap_bytes_ - heap_size) {
    return nullptr;
  }
  ...
  TrackMetalHeapCreated(heap_size);
  return heap;
}
