...
bool MetalTextureCache::TryGpuLoadTexture(Texture& texture, bool load_base,
  ...
  if (use_blit_upload) {
    ...
    cmd->retain();
    cmd->addCompletedHandler([](MTL::CommandBuffer* cb) { cb->release(); });
    cmd->commit();
  } else {
    cmd->commit();
...
void MetalTextureCache::RecordCacheTextureCreated(MTL::Texture* texture) {
  ...
  cache_texture_bytes_created_.fetch_add(EstimateMetalTextureBytes(texture),
...
MTL::Texture* MetalTextureCache::GetTextureForBinding(
    ...
    if (fetch_constant < logged_missing_binding.size() &&
      ...
      if (decoded_key.is_valid) {
        ...
        const char* format_name =
            format_info ? FormatInfo::GetName(decoded_key.format) : "unknown";
        XELOGW(
  ...
  MTL::Texture* result = metal_texture
                             ? metal_texture->GetOrCreateView(
