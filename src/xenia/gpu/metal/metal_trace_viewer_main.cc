...
class MetalTraceViewer final : public TraceViewer {
 public:
  ...
  uintptr_t GetTextureEntry(const TextureInfo& texture_info,
    ...
    MTL::Texture* texture =
        texture_cache->GetTextureForBinding(fetch_constant, dimension, false);
 ...
 private:
  ...
  uintptr_t WrapTexture(MTL::Texture* texture, MTL::SamplerState* sampler,
    ...
    auto* metal_drawer =
        dynamic_cast<xe::ui::metal::MetalImmediateDrawer*>(immediate_drawer());
    if (!metal_drawer) {
      return 0;
    }
  ...
  std::unordered_map<TextureCacheKey, std::unique_ptr<xe::ui::ImmediateTexture>,
